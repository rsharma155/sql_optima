...
func (w *TimescaleWriter) WriteMSSQLMetrics(ctx context.Context, metrics []domain.MSSQLCombinedMetric) error {
	...
	batch := `INSERT INTO sqlserver_query_metrics_v2 (
...
func (w *TimescaleWriter) WritePGMetrics(ctx context.Context, metrics []domain.PGCombinedMetric) error {
	...
	batch := `INSERT INTO pg_query_metrics_v2 (
		ts, instance_id, datname, usename, application_name,
	...
	for _, m := range metrics {
		_, err := w.pool.Exec(ctx, batch,
			m.Timestamp, m.InstanceID, m.DatabaseName, m.UserName, m.ApplicationName,
