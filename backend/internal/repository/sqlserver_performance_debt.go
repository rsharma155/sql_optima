...
func (c *SqlServerRepository) FetchUnusedIndexes(instanceName, databaseName string, minUpdates int64, limit int) ([]PerformanceDebtUnusedIndex, error) {
	...
	rows, err := db.Query(q)
	if err != nil {
		log.Printf("[SQLSERVER] FetchUnusedIndexes error for %s/%s: %v", instanceName, databaseName, err)
		return nil, err
	}
	defer rows.Close()
...
func (c *SqlServerRepository) FetchMissingIndexRecommendations(instanceName, databaseName string, limit int) ([]PerformanceDebtMissingIndex, error) {
	db, ok := c.GetConn(instanceName)
	if !ok || db == nil {
		return nil, fmt.Errorf("no connection for instance: %s", instanceName)
	}
	...
	rows, err := db.Query(q)
	if err != nil {
		log.Printf("[SQLSERVER] FetchMissingIndexRecommendations error for %s/%s: %v", instanceName, databaseName, err)
		return nil, err
	}
	defer rows.Close()
...
func (c *SqlServerRepository) FetchIndexFragmentation(instanceName, databaseName string, minFragPct float64, minPages int64, limit int) ([]PerformanceDebtIndexFrag, error) {
	db, ok := c.GetConn(instanceName)
	if !ok || db == nil {
		return nil, fmt.Errorf("no connection for instance: %s", instanceName)
	}
	...
	rows, err := db.Query(q)
	if err != nil {
		log.Printf("[SQLSERVER] FetchIndexFragmentation error for %s/%s: %v", instanceName, databaseName, err)
		return nil, err
	}
	defer rows.Close()
...
func (c *SqlServerRepository) FetchStaleStatistics(instanceName, databaseName string, modificationPct float64, limit int) ([]PerformanceDebtStaleStats, error) {
	db, ok := c.GetConn(instanceName)
	if !ok || db == nil {
		return nil, fmt.Errorf("no connection for instance: %s", instanceName)
	}
	...
	rows, err := db.Query(q)
	if err != nil {
		log.Printf("[SQLSERVER] FetchStaleStatistics error for %s/%s: %v", instanceName, databaseName, err)
		return nil, err
	}
	defer rows.Close()
...
func (c *SqlServerRepository) FetchAutogrowthRisks(instanceName, databaseName string, limit int) ([]PerformanceDebtFileGrowth, error) {
	db, ok := c.GetConn(instanceName)
	if !ok || db == nil {
		return nil, fmt.Errorf("no connection for instance: %s", instanceName)
	}
	...
	rows, err := db.Query(q)
	if err != nil {
		log.Printf("[SQLSERVER] FetchAutogrowthRisks error for %s/%s: %v", instanceName, databaseName, err)
		return nil, err
	}
	defer rows.Close()
...
func (c *SqlServerRepository) FetchVLFCount(instanceName, databaseName string) (int64, error) {
	db, ok := c.GetConn(instanceName)
	if !ok || db == nil {
	...
	if err := db.QueryRow(q).Scan(&n); err != nil {
		log.Printf("[SQLSERVER] FetchVLFCount error for %s/%s: %v", instanceName, databaseName, err)
		return 0, err
	}
	return n, nil
}

func (c *SqlServerRepository) FetchLastFullBackupAgeHours(instanceName, databaseName string) (float64, error) {
	db, ok := c.GetConn(instanceName)
	if !ok || db == nil {
		return 0, fmt.Errorf("no connection for instance: %s", instanceName)
	}
...
func (c *SqlServerRepository) FetchFailedAgentJobs24h(instanceName string, limit int) ([]map[string]interface{}, error) {
	db, ok := c.GetConn(instanceName)
	if !ok || db == nil {
		return nil, fmt.Errorf("no connection for instance: %s", instanceName)
	}
...
func (c *SqlServerRepository) FetchDisabledAgentJobs(instanceName string, limit int) ([]string, error) {
	db, ok := c.GetConn(instanceName)
	if !ok || db == nil {
		return nil, fmt.Errorf("no connection for instance: %s", instanceName)
	}
...
func (c *SqlServerRepository) FetchConfigValueInUse(instanceName string, name string) (int64, error) {
	db, ok := c.GetConn(instanceName)
	if !ok || db == nil {
		return 0, fmt.Errorf("no connection for instance: %s", instanceName)
	}
