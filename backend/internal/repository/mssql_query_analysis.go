...
func (c *MssqlRepository) FetchQueryRegressions(instanceName string) ([]models.MssqlQueryRegression, error) {
	...
	if err != nil || len(dbNames) == 0 {
		log.Printf("[MSSQL] FetchQueryRegressions: no QS databases for %s: %v", instanceName, err)
		return nil, nil
	}

	var all []models.MssqlQueryRegression
	for _, dbn := range dbNames {
		rows, err := fetchRegressionsForDB(db, dbn)
		if err != nil {
			log.Printf("[MSSQL] FetchQueryRegressions(%s/%s): %v", instanceName, dbn, err)
			continue
		}
		all = append(all, rows...)
	}
...
func fetchRegressionsForDB(db *sql.DB, dbName string) ([]models.MssqlQueryRegression, error) {
	...
	var results []models.MssqlQueryRegression
	for rows.Next() {
		var r models.MssqlQueryRegression
		...
		if err := rows.Scan(&r.QueryHash, &r.QueryText, &regType,
			...
			log.Printf("[MSSQL] FetchQueryRegressions scan: %v", err)
			continue
		}
...
func (c *MssqlRepository) FetchPlanInstability(instanceName string) ([]models.MssqlPlanInstability, error) {
	db, ok := c.GetConn(instanceName)
	if !ok || db == nil {
		return nil, fmt.Errorf("no connection for instance: %s", instanceName)
	}
	...
	dbNames, err := c.listUserDatabaseNamesForQueryStore(db)
	if err != nil || len(dbNames) == 0 {
		log.Printf("[MSSQL] FetchPlanInstability: no QS databases for %s: %v", instanceName, err)
		return nil, nil
	}

	var all []models.MssqlPlanInstability
	for _, dbn := range dbNames {
		rows, err := fetchPlanInstabilityForDB(db, dbn)
		if err != nil {
			log.Printf("[MSSQL] FetchPlanInstability(%s/%s): %v", instanceName, dbn, err)
			continue
		}
		all = append(all, rows...)
	}
...
func fetchPlanInstabilityForDB(db *sql.DB, dbName string) ([]models.MssqlPlanInstability, error) {
	...
	var results []models.MssqlPlanInstability
	for rows.Next() {
		var r models.MssqlPlanInstability
		var lastExec sql.NullTime
		if err := rows.Scan(&r.QueryHash, &r.QueryText, &r.PlanCount, &lastExec); err != nil {
			log.Printf("[MSSQL] FetchPlanInstability scan: %v", err)
			continue
		}
		r.DatabaseName = dbName
		if lastExec.Valid {
...
func (c *MssqlRepository) FetchWatchedQueryStats(ctx context.Context, instanceName, dbName, queryHash string) (*models.MssqlWatchedQuerySnapshot, error) {
	if dbName == "" {
	...
	var s models.MssqlWatchedQuerySnapshot
	var lastExec sql.NullTime
...
func (c *MssqlRepository) FetchQueryPlans(ctx context.Context, instanceName, dbName, queryHash string) ([]models.MssqlQueryPlanInfo, error) {
	db, ok := c.GetConn(instanceName)
	if !ok || db == nil {
		return nil, fmt.Errorf("no connection for instance: %s", instanceName)
	}
	...
	var results []models.MssqlQueryPlanInfo
	for rows.Next() {
		var p models.MssqlQueryPlanInfo
		...
		if err := rows.Scan(&p.PlanID, &p.AvgDurationMs, &p.AvgCpuMs, &p.AvgReads,
			...
			log.Printf("[MSSQL] FetchQueryPlans scan: %v", err)
			continue
		}
		if lastExec.Valid {
...
func (c *MssqlRepository) FetchQueryWaitStats(ctx context.Context, instanceName, dbName, queryHash string) ([]models.MssqlQueryWaitStat, error) {
	db, ok := c.GetConn(instanceName)
	if !ok || db == nil {
		return nil, fmt.Errorf("no connection for instance: %s", instanceName)
	}
	...
	if err != nil {
		...
		log.Printf("[MSSQL] FetchQueryWaitStats(%s): %v — may not be supported", instanceName, err)
		return nil, nil
	}
	defer rows.Close()

	var results []models.MssqlQueryWaitStat
	for rows.Next() {
		var w models.MssqlQueryWaitStat
		if err := rows.Scan(&w.WaitCategory, &w.AvgWaitMs, &w.TotalWaitMs); err != nil {
			log.Printf("[MSSQL] FetchQueryWaitStats scan: %v", err)
			continue
		}
