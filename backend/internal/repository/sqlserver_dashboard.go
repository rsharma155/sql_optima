...
func (c *SqlServerRepository) FetchLiveTelemetry(instanceName string, prev models.DashboardMetrics) models.DashboardMetrics {
	...
	cacheSQL := `
		SELECT /* SQL_OPTIMA */   TOP 20
			...
			ISNULL(t.text, 'Unknown') as query_text,
...
func (c *SqlServerRepository) FetchTopCPUQueries(instanceName string, limit int, database string) ([]map[string]interface{}, error) {
	c.mutex.RLock()
	db, ok := c.conns[instanceName]
	c.mutex.RUnlock()
	...
	OUTER APPLY (
		SELECT /* SQL_OPTIMA */   CONVERT(INT, value) AS dbid
	...
	if err != nil {
		log.Printf("[SQLSERVER] FetchTopCPUQueries Error for %s: %v", instanceName, err)
		return nil, err
	}
	defer rows.Close()
	...
	log.Printf("[SQLSERVER] FetchTopCPUQueries: raw query returned %d rows for %s", len(rawResults), instanceName)
