...
func (c *PgRepository) CollectPgSystemStats(ctx context.Context, db *sql.DB) (float64, float64, error) {
	// First check if the extension functions exist to avoid query planning errors
	var hasProctab bool
	ctx, cancel := WithQueryTimeout(ctx, 0)
	defer cancel()
	_ = db.QueryRowContext(ctx, "/* SQL_OPTIMA */   SELECT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'pg_stat_cpu')").Scan(&hasProctab)
	...
	ctx, cancel = WithQueryTimeout(ctx, 0)
	defer cancel()
	err := db.QueryRowContext(ctx, query).Scan(&memUsage, &cpuUsage)
	if err != nil {
		slog.Error("[PostgreSQL] System Stats Query Error (despite proc check)", "err", err)
		return 0, 0, nil
	}
...
func (c *PgRepository) CollectPgServerInfo(ctx context.Context, db *sql.DB) (string, string, error) {
	query := `
		/* SQL_OPTIMA */
		SELECT /* SQL_OPTIMA */   
			version() AS version,
	...
	ctx, cancel := WithQueryTimeout(ctx, 0)
	defer cancel()
	err := db.QueryRowContext(ctx, query).Scan(&version, &uptime)
	if err != nil {
		slog.Error("[PostgreSQL] Server Info Query Error", "err", err)
	}
