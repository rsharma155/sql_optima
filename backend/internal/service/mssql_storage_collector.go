...
// StartMssqlStorageHistoryCollector starts the background task to collect storage snapshots.
func (s *MetricsService) StartMssqlStorageHistoryCollector(ctx context.Context) {
	log.Println("[Collector][MSSQL-Storage] History collector started (daily cadence)")
	...
	time.AfterFunc(1*time.Minute, func() {
		s.RunMssqlStorageSnapshotCollection(ctx)
	})
...
// RunMssqlStorageSnapshotCollection performs a full sweep of monitored SQL instances.
func (s *MetricsService) RunMssqlStorageSnapshotCollection(ctx context.Context) {
	if s.tsLogger == nil {
		return
	}
	...
	for _, inst := range s.Config.Instances {
		...
		log.Printf("[Collector][MSSQL-Storage] Collecting snapshots for instance: %s", inst.Name)
