...
// SqlServerBlockingEvaluator checks for active blocking chains in SQL Server.
...
func (e *SqlServerBlockingEvaluator) Evaluate(ctx context.Context, instanceName string) ([]AlertEvaluatorResult, error) {
	const q = `
	...
	if err := e.tsPool.QueryRow(ctx, q, instanceName).Scan(&blocking); err != nil {
		...
		return nil, fmt.Errorf("sqlserver_blocking: %w", err)
	}
	if blocking == 0 {
		return nil, nil
	}
