...
func (s *MetricsService) collectQueryAnalysisData(ctx context.Context) {
	...
	for _, inst := range s.Config.Instances {
		...
		if err != nil {
			...
			rows := make([]hot.MssqlQueryRegressionRow, len(regs))
			for i, r := range regs {
				rows[i] = hot.MssqlQueryRegressionRow{
			...
			if err := s.tsLogger.LogMssqlQueryRegressions(ctx, rows); err != nil {
				log.Printf("[QueryAnalysis] LogMssqlQueryRegressions(%s): %v", inst.Name, err)
			}
		}
		...
		if err != nil {
			...
			rows := make([]hot.MssqlPlanInstabilityRow, len(pis))
			for i, p := range pis {
				rows[i] = hot.MssqlPlanInstabilityRow{
			...
			if err := s.tsLogger.LogMssqlPlanInstability(ctx, rows); err != nil {
				log.Printf("[QueryAnalysis] LogMssqlPlanInstability(%s): %v", inst.Name, err)
			}
		}
	}
}
...
func (s *MetricsService) collectWatchedQuerySnapshots(ctx context.Context) {
	...
	for _, inst := range s.Config.Instances {
		...
		wqs, err := s.tsLogger.ListMssqlWatchedQueries(ctx, inst.Name)
		if err != nil {
			log.Printf("[WatchedQuery] ListMssqlWatchedQueries(%s): %v", inst.Name, err)
			continue
		}
		if len(wqs) == 0 {
			continue
		...
		var snapRows []hot.MssqlWatchedSnapshotRow
		for _, wq := range wqs {
			...
			if wq.QueryText == "" && snap.QueryText != "" {
				_ = s.tsLogger.UpdateMssqlWatchedQueryText(ctx, wq.ID, snap.QueryText)
			}
			...
			snapRows = append(snapRows, hot.MssqlWatchedSnapshotRow{
				SnapshotTime:      now,
				WatchedID:         wq.ID,
		...
		if len(snapRows) > 0 {
			if err := s.tsLogger.LogMssqlWatchedQuerySnapshot(ctx, snapRows); err != nil {
				log.Printf("[WatchedQuery] LogMssqlWatchedQuerySnapshot(%s): %v", inst.Name, err)
			}
		}
	}
}
