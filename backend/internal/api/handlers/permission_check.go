...
func buildPGCreateUserScript(username string) string {
	...
	fmt.Fprintf(&b, "--   ALTER ROLE %s PASSWORD 'your-secret-from-vault';\n", quoted)
	b.WriteString("-- ============================================================\n\n")

	b.WriteString("DO $$\n")
	b.WriteString("BEGIN\n")
	fmt.Fprintf(&b, "    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = '%s') THEN\n", escapeSQLStr(raw))
	fmt.Fprintf(&b, "        CREATE ROLE %s WITH\n", quoted)
	b.WriteString("            LOGIN\n")
	...
	fmt.Fprintf(&b, "        RAISE NOTICE 'Role %s created — set password with ALTER ROLE %s PASSWORD ''...''';\n", quoted, quoted)
	b.WriteString("    ELSE\n")
	fmt.Fprintf(&b, "        RAISE NOTICE 'Role %s already exists.';\n", quoted)
	b.WriteString("    END IF;\n")
	b.WriteString("END\n")
	b.WriteString("$$;\n\n")

	b.WriteString("-- Grant required system roles\n")
	fmt.Fprintf(&b, "GRANT pg_read_all_settings TO %s;\n", quoted)
	fmt.Fprintf(&b, "GRANT pg_read_all_stats TO %s;\n", quoted)
	fmt.Fprintf(&b, "GRANT pg_stat_scan_tables TO %s;\n", quoted)
	fmt.Fprintf(&b, "GRANT pg_monitor TO %s;\n\n", quoted)

	b.WriteString("-- Grant SELECT on key monitoring views\n")
	fmt.Fprintf(&b, "GRANT SELECT ON pg_stat_activity TO %s;\n", quoted)
	fmt.Fprintf(&b, "GRANT SELECT ON pg_stat_bgwriter TO %s;\n", quoted)
	fmt.Fprintf(&b, "GRANT SELECT ON pg_stat_database TO %s;\n", quoted)
	fmt.Fprintf(&b, "GRANT SELECT ON pg_stat_user_indexes TO %s;\n", quoted)
	fmt.Fprintf(&b, "GRANT SELECT ON pg_stat_replication TO %s;\n", quoted)
	fmt.Fprintf(&b, "GRANT SELECT ON pg_replication_slots TO %s;\n", quoted)
	fmt.Fprintf(&b, "GRANT SELECT ON pg_locks TO %s;\n\n", quoted)

	b.WriteString("-- Grant SELECT on pg_catalog system tables\n")
	fmt.Fprintf(&b, "GRANT SELECT ON pg_catalog.pg_stat_activity TO %s;\n", quoted)
	fmt.Fprintf(&b, "GRANT SELECT ON pg_catalog.pg_stat_bgwriter TO %s;\n", quoted)
	fmt.Fprintf(&b, "GRANT SELECT ON pg_catalog.pg_stat_database TO %s;\n", quoted)
	fmt.Fprintf(&b, "GRANT SELECT ON pg_catalog.pg_stat_user_tables TO %s;\n", quoted)
	fmt.Fprintf(&b, "GRANT SELECT ON pg_catalog.pg_stat_user_indexes TO %s;\n", quoted)
	fmt.Fprintf(&b, "GRANT SELECT ON pg_catalog.pg_stat_sys_tables TO %s;\n", quoted)
	fmt.Fprintf(&b, "GRANT SELECT ON pg_catalog.pg_statio_user_tables TO %s;\n", quoted)
	fmt.Fprintf(&b, "GRANT SELECT ON pg_catalog.pg_statio_sys_tables TO %s;\n", quoted)
	fmt.Fprintf(&b, "GRANT SELECT ON pg_catalog.pg_locks TO %s;\n", quoted)
	fmt.Fprintf(&b, "GRANT SELECT ON pg_catalog.pg_stat_replication TO %s;\n", quoted)
	fmt.Fprintf(&b, "GRANT SELECT ON pg_catalog.pg_replication_origin_status TO %s;\n", quoted)
	fmt.Fprintf(&b, "GRANT SELECT ON pg_catalog.pg_replication_slots TO %s;\n", quoted)
	fmt.Fprintf(&b, "GRANT SELECT ON pg_catalog.pg_settings TO %s;\n", quoted)
	fmt.Fprintf(&b, "GRANT SELECT ON pg_catalog.pg_roles TO %s;\n", quoted)
	fmt.Fprintf(&b, "GRANT SELECT ON pg_catalog.pg_database TO %s;\n", quoted)
	fmt.Fprintf(&b, "GRANT SELECT ON pg_catalog.pg_namespace TO %s;\n", quoted)
	fmt.Fprintf(&b, "GRANT SELECT ON pg_catalog.pg_class TO %s;\n", quoted)
	fmt.Fprintf(&b, "GRANT SELECT ON pg_catalog.pg_attribute TO %s;\n", quoted)
	fmt.Fprintf(&b, "GRANT SELECT ON pg_catalog.pg_proc TO %s;\n", quoted)
	fmt.Fprintf(&b, "GRANT SELECT ON pg_catalog.pg_type TO %s;\n", quoted)
	fmt.Fprintf(&b, "GRANT SELECT ON pg_catalog.pg_index TO %s;\n", quoted)
	fmt.Fprintf(&b, "GRANT SELECT ON pg_catalog.pg_inherits TO %s;\n", quoted)
	fmt.Fprintf(&b, "GRANT SELECT ON pg_catalog.pg_tablespace TO %s;\n\n", quoted)

	b.WriteString("-- Grant access to information_schema\n")
	fmt.Fprintf(&b, "GRANT SELECT ON information_schema.tables TO %s;\n", quoted)
	fmt.Fprintf(&b, "GRANT SELECT ON information_schema.columns TO %s;\n", quoted)
	fmt.Fprintf(&b, "GRANT SELECT ON information_schema.views TO %s;\n", quoted)
	fmt.Fprintf(&b, "GRANT SELECT ON information_schema.routines TO %s;\n", quoted)
	fmt.Fprintf(&b, "GRANT SELECT ON information_schema.table_privileges TO %s;\n", quoted)
	fmt.Fprintf(&b, "GRANT SELECT ON information_schema.column_privileges TO %s;\n\n", quoted)
	...
	fmt.Fprintf(&b, "        EXECUTE 'GRANT SELECT ON pg_stat_statements TO %s';\n", quoted)
	fmt.Fprintf(&b, "        RAISE NOTICE 'Granted pg_stat_statements access to %s';\n", quoted)
	b.WriteString("    ELSE\n")
...
func buildMSSQLCreateUserScript(username string) string {
	...
	fmt.Fprintf(&b, "    PRINT 'Login %s created.';\n", bracketed)
	b.WriteString("END\nELSE\nBEGIN\n")
	fmt.Fprintf(&b, "    PRINT 'Login %s already exists.';\n", bracketed)
	b.WriteString("END\nGO\n\n")

	b.WriteString("-- Grant server-level permissions\n")
	fmt.Fprintf(&b, "GRANT VIEW SERVER STATE TO %s;\n", bracketed)
	fmt.Fprintf(&b, "GRANT VIEW ANY DEFINITION TO %s;\n", bracketed)
	b.WriteString("GO\n\n")

	b.WriteString("-- Create user in msdb for SQL Agent job monitoring\n")
	b.WriteString("USE msdb;\nGO\n\n")
	fmt.Fprintf(&b, "IF NOT EXISTS (SELECT name FROM sys.database_principals WHERE name = '%s')\nBEGIN\n", escapeSQLStr(username))
	fmt.Fprintf(&b, "    CREATE USER %s FOR LOGIN %s;\n", bracketed, bracketed)
	fmt.Fprintf(&b, "    PRINT 'User %s created in msdb.';\n", bracketed)
	b.WriteString("END\nGO\n\n")

	b.WriteString("-- Grant SELECT on SQL Agent tables\n")
	fmt.Fprintf(&b, "GRANT SELECT ON dbo.sysjobs TO %s;\n", bracketed)
	fmt.Fprintf(&b, "GRANT SELECT ON dbo.sysjobschedules TO %s;\n", bracketed)
	fmt.Fprintf(&b, "GRANT SELECT ON dbo.sysjobactivity TO %s;\n", bracketed)
	fmt.Fprintf(&b, "GRANT SELECT ON dbo.sysjobhistory TO %s;\n", bracketed)
	fmt.Fprintf(&b, "GRANT SELECT ON dbo.sysschedules TO %s;\n", bracketed)
	fmt.Fprintf(&b, "GRANT SELECT ON dbo.syscategories TO %s;\n", bracketed)
	fmt.Fprintf(&b, "GRANT SELECT ON dbo.sysjobsteps TO %s;\n", bracketed)
	fmt.Fprintf(&b, "GRANT SELECT ON dbo.sysoperators TO %s;\n", bracketed)
	b.WriteString("GO\n\n")

	b.WriteString("-- Add to SQLAgentReaderRole for enhanced job visibility\n")
	fmt.Fprintf(&b, "EXEC sp_addrolemember 'SQLAgentReaderRole', '%s';\n", escapeSQLStr(username))
	b.WriteString("GO\n\n")
	...
	fmt.Fprintf(&b, "    CREATE USER %s FOR LOGIN %s;\n", bracketed, bracketed)
	b.WriteString("END\n")
	fmt.Fprintf(&b, "GRANT SELECT ON SCHEMA::dbo TO %s;\n", bracketed)
	b.WriteString("GO\n*/\n")
