...
func TestComputeWalRateMBPerMin_FirstObservationReturnsNotOk(t *testing.T) {
	...
	id := uuid.New()
	if rate, ok := tl.ComputeWalRateMBPerMin(id, 1000, 60); ok || rate != 0 {
...
func TestComputeWalRateMBPerMin_ComputesRateAndClampsNegative(t *testing.T) {
	tl := NewTimescaleLogger(nil)
	id := uuid.New()
	_, _ = tl.ComputeWalRateMBPerMin(id, 100*1024*1024, 60) // baseline

	rate, ok := tl.ComputeWalRateMBPerMin(id, 160*1024*1024, 60)
	if !ok {
	...
	rate2, ok2 := tl.ComputeWalRateMBPerMin(id, 10, 60) // counter reset -> clamp
	if !ok2 {
