...
//
// Purpose: PostgreSQL backup status and history logger.
//
// Author: Ravi Sharma
...
func (tl *TimescaleLogger) GetLatestPostgresBackupRun(ctx context.Context, serverID uuid.UUID) (*PostgresBackupRunRow, error) {
	q := `
	...
	if err := tl.pool.QueryRow(ctx, q, serverID).Scan(
...
func (tl *TimescaleLogger) GetPostgresBackupRunHistory(ctx context.Context, serverID uuid.UUID, limit int) ([]PostgresBackupRunRow, error) {
	if limit <= 0 {
		limit = 100
	}
	q := `
