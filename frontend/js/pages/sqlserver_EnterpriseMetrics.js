    ...
    async function loadMetrics() {
        try {
            ...
            const [latchStats, waitingTasks, memoryGrants, schedulerWG, procedureStats, fileIoLatency, spinlockStats, memoryClerks, tempdbStats, planCacheHealth, memoryGrantWaiters, tempdbTopConsumers, waitCategories] = await Promise.all([
                fetchWithError(`/api/sqlserver/latch-stats?instance=${encodeURIComponent(inst.name)}`),
                fetchWithError(`/api/sqlserver/waiting-tasks?instance=${encodeURIComponent(inst.name)}`),
                fetchWithError(`/api/sqlserver/memory-grants?instance=${encodeURIComponent(inst.name)}`),
                fetchWithError(`/api/sqlserver/scheduler-wg?instance=${encodeURIComponent(inst.name)}`),
                fetchWithError(`/api/sqlserver/procedure-stats?instance=${encodeURIComponent(inst.name)}`),
                fetchWithError(`/api/sqlserver/file-io-latency?instance=${encodeURIComponent(inst.name)}`),
                fetchWithError(`/api/sqlserver/spinlock-stats?instance=${encodeURIComponent(inst.name)}`),
                fetchWithError(`/api/sqlserver/memory-clerks?instance=${encodeURIComponent(inst.name)}`),
                fetchWithError(`/api/sqlserver/tempdb-stats?instance=${encodeURIComponent(inst.name)}`),
                fetchWithError(`/api/sqlserver/plan-cache-health?instance=${encodeURIComponent(inst.name)}`),
                fetchWithError(`/api/sqlserver/memory-grant-waiters?instance=${encodeURIComponent(inst.name)}`),
                fetchWithError(`/api/sqlserver/tempdb-top-consumers?instance=${encodeURIComponent(inst.name)}`),
                fetchWithError(`/api/sqlserver/wait-categories?instance=${encodeURIComponent(inst.name)}`)
            ]);
