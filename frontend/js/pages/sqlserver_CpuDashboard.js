...
window.SqlServerCpuDashboardView = async function() {
        ...
        <div class="page-view active dashboard-sky-theme">
            <div class="page-title flex-between">
                ...
                <div class="flex-between" style="align-items:center; gap:1rem;">
                    ...
                    <button class="btn btn-sm btn-outline text-accent" data-action="call" data-fn="refreshSqlServerCpuDashboard"><i class="fa-solid fa-refresh"></i> Refresh</button>
                </div>
            </div>
    ...
    await window.loadSqlServerCpuDashboard();
};

window.loadSqlServerCpuDashboard = async function() {
    const inst = window.appState.config.instances[window.appState.currentInstanceIdx];
    ...
    try {
        ...
        const [propertiesRes, schedulerRes] = await Promise.all([
            window.apiClient.authenticatedFetch(`/api/sqlserver/server-properties?instance=${encodeURIComponent(inst.name)}`),
            window.apiClient.authenticatedFetch(`/api/sqlserver/cpu-scheduler-stats?instance=${encodeURIComponent(inst.name)}&limit=${limit}`)
        ]);
...
window.refreshSqlServerCpuDashboard = function() {
    window.SqlServerCpuDashboardView();
};