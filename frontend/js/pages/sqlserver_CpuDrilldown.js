...
window.loadQueryStatsWithMetric = async function(instanceName, metric, timeRange, fromLocal, toLocal) {
    ...
    try {
        let apiUrl = `/api/timescale/sqlserver/query-stats-dashboard?instance=${encodeURIComponent(instanceName)}&metric=${encodeURIComponent(metric)}&time_range=${encodeURIComponent(timeRange)}&dimension=query&limit=50`;
        if (fromISO && toISO) {
...
window.loadCpuDrilldownDataLegacy = async function(instanceName, fromLocal, toLocal) {
    ...
    try {
        let queriesUrl = `/api/sqlserver/cpu-drilldown?instance=${encodeURIComponent(instanceName)}&limit=200`;
        const fromISO = window.cpuDrilldownLocalToRFC3339(fromLocal);
        const toISO = window.cpuDrilldownLocalToRFC3339(toLocal);
        if (fromISO && toISO) {
    ...
    try {
        ...
        if (fromISO && toISO) {
            const tsRes = await window.apiClient.authenticatedFetch(
                `/api/timescale/sqlserver/cpu-history?instance=${encodeURIComponent(instanceName)}&from=${encodeURIComponent(fromISO)}&to=${encodeURIComponent(toISO)}`
            );
            if (tsRes.ok) {
            ...
            const dashRes = await window.apiClient.authenticatedFetch(`/api/sqlserver/dashboard?instance=${encodeURIComponent(instanceName)}`);
            if (dashRes.ok) {
...
window.loadCpuDrilldownData = async function(instanceName, fromTime, toTime) {
    try {
        let cpuUrl = `/api/timescale/sqlserver/cpu-history?instance=${encodeURIComponent(instanceName)}`;
        // Top queries: API prefers TimescaleDB (/api/sqlserver/cpu-drilldown); use Real-Time Diagnostics for live DMV.
        let queriesUrl = `/api/sqlserver/cpu-drilldown?instance=${encodeURIComponent(instanceName)}&limit=200`;
        ...
        const [dashRes, topQueriesRes] = await Promise.all([
            window.apiClient.authenticatedFetch(`/api/sqlserver/dashboard?instance=${encodeURIComponent(instanceName)}`),
