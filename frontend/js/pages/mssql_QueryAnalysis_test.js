...
(function() {
    console.log('%c [TEST] Starting mssql_QueryAnalysis smoke test...', 'color: #3b82f6; font-weight: bold;');
    ...
    async function runTest() {
        try {
            if (typeof window.mssql_QueryAnalysisDashboard !== 'function') {
                throw new Error('mssql_QueryAnalysisDashboard function not found on window');
            }

            console.log('[TEST] Executing dashboard load...');
            await window.mssql_QueryAnalysisDashboard();
    ...
    // Export runner
    window.runMssqlQASmokeTest = runTest;
    console.log('[TEST] Type %crunMssqlQASmokeTest()%c in console to execute.', 'font-family: monospace; font-weight: bold; background: #eee;', '');
})();
