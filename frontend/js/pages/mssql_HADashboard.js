...
async function fetchAGData(instanceName, from, to) {
    ...
    try {
        const [respHealth, respHist] = await Promise.all([
            ...
            window.apiClient.authenticatedFetch(`/api/mssql/ag-health/history?instance=${encodeURIComponent(instanceName)}&from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}`)
        ]);
...
async function fetchReplData(instanceName) {
    ...
    try {
        const resp = await window.apiClient.authenticatedFetch(`/api/mssql/replication-status?instance=${encodeURIComponent(instanceName)}`);
...
async function fetchLSData(instanceName) {
    ...
    try {
        const resp = await window.apiClient.authenticatedFetch(`/api/mssql/log-shipping?instance=${encodeURIComponent(instanceName)}`);
        const data = await resp.json();
