...
(function () {
    ...
    async function loadWatchedQueryAnalyzer() {
        ...
        outlet.innerHTML = await window.loadTemplate('/pages/mssql_watched_queries.html');
            ...
            try {
                ...
                body.querySelectorAll('.wq-delete-btn').forEach(btn => {
                    btn.addEventListener('click', async () => {
                        ...
                        await window.apiClient.authenticatedFetch(`/api/mssql/watched-queries?instance=${encodeURIComponent(instName)}&id=${btn.dataset.id}`, { method: 'DELETE' });
                        fetchList();
                    });
                });
            } catch (err) {
        ...
        async function fetchSnapshots(id) {
            try {
                const resp = await window.apiClient.authenticatedFetch(`/api/mssql/watched-queries/detail?instance=${encodeURIComponent(instName)}&id=${id}`, { cache: 'no-store' });
                const data = await resp.json();
        ...
        async function fetchPlans(queryHash, dbName) {
            ...
            try {
                const resp = await window.apiClient.authenticatedFetch(`/api/mssql/query-analysis/query-plans?instance=${encodeURIComponent(instName)}&database=${encodeURIComponent(dbName||'master')}&query_hash=${encodeURIComponent(queryHash)}`, { cache: 'no-store' });
                const data = await resp.json();
        ...
        async function fetchWaits(queryHash, dbName) {
            ...
            try {
                const resp = await window.apiClient.authenticatedFetch(`/api/mssql/query-analysis/query-wait-stats?instance=${encodeURIComponent(instName)}&database=${encodeURIComponent(dbName||'master')}&query_hash=${encodeURIComponent(queryHash)}`, { cache: 'no-store' });
                const data = await resp.json();
    ...
    window.mssql_WatchedQueryAnalyzer = loadWatchedQueryAnalyzer;
})();
