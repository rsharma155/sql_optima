...
window.PgQueriesView = async function() {
    ...
    window.pgQueriesInterval = window.registerInterval(() => {
        if (window.appState.activeViewId === 'pg-queries') {
            initPgQueries();
        } else {
            clearInterval(window.pgQueriesInterval);
            window.pgQueriesInterval = null;
        }
...
window.pgssHeaderSort = function(key) {
    ...
    // Update header icons
    document.querySelectorAll('#pgss-top-grid .pgss-sortable-th').forEach(th => {
...
window.pgssRegressionSort = function(key) {
    ...
    if (window._pgssRegrSort === key) window._pgssRegrSortDir = window._pgssRegrSortDir === 'desc' ? 'asc' : 'desc';
    else { window._pgssRegrSort = key; window._pgssRegrSortDir = 'desc'; }
    const field = keyMap[key] || key; const dir = window._pgssRegrSortDir === 'desc' ? -1 : 1;
...
function pgssRenderRegressionRows(regs) {
    const tbody = document.getElementById('pgss-regression-tbody');
    if (!tbody) return;
    if (!regs || regs.length === 0) { tbody.innerHTML = '<tr><td colspan="6" class="text-center text-muted">No regressions.</td></tr>'; return; }
    tbody.innerHTML = regs.map(r => {
        const decodedSql = pgssSmartDecode(r.query || '');
        return `<tr><td style="max-width:420px; cursor:pointer; text-decoration:underline;" data-action="call" data-fn="pgssOpenRegressionQuery" data-arg="${pgssEscapeHtml(decodedSql)}">${pgssEscapeHtml(pgssTrancate(decodedSql, 80))}</td>` +
               `<td class="text-right">${pgssFmtMs(r.prev_avg_ms)}</td><td class="text-right">${pgssFmtMs(r.curr_avg_ms)}</td>` +
               `<td class="text-right ${r.change_pct > 50 ? 'text-danger' : ''}">+${r.change_pct.toFixed(0)}%</td>` +
               `<td class="text-center"><span class="badge ${r.status === 'Degraded' ? 'badge-danger' : 'badge-warning'}">${r.status}</span></td>` +
               `<td class="text-right">${r.detected_at ? new Date(r.detected_at).toLocaleTimeString() : '-'}</td></tr>`;
    }).join('');
}
window.togglePgssWideView = function() {
    ...
    if (btn) btn.innerHTML = grid.classList.contains('wide-view') ? '<i class="fa-solid fa-compress"></i> Standard' : '<i class="fa-solid fa-expand"></i> Wide View';
};
