...
wait_agg AS (
    ...
    WHERE server_id = $1
    GROUP BY date_trunc('minute', capture_timestamp), server_id
)
SELECT 
    m.capture_timestamp, m.server_id, m.avg_cpu_load, m.memory_usage,
...
LEFT JOIN wait_agg w ON date_trunc('minute', m.capture_timestamp) = w.bucket AND m.server_id = w.server_id
