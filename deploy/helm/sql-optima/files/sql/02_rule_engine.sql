...
('PG_SEQUENCE_EXHAUSTION_033', 'Sequence Exhaustion Risk', 'Schema', 'Database', 'Critical', 'BestPractice', 'Checks sequences approaching their maximum value (80% threshold).', 'SELECT COUNT(*) AS cnt FROM pg_sequences WHERE max_value > 0 AND last_value IS NOT NULL AND (last_value::numeric - min_value::numeric + 1) / NULLIF((max_value::numeric - min_value::numeric + 1), 0) * 100 >= 80;', NULL, 'cnt == 0 ? "OK" : "Critical"', '0', '0', NULL, 'ALTER SEQUENCE <seq> AS BIGINT;', 'threshold', '{"max": 0}', 33, 'postgres', TRUE, NULL, '{"category": "availability"}', 'definitive')
